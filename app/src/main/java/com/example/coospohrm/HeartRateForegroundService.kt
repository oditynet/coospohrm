...
class HeartRateForegroundService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O &&
            ...
            nm.createNotificationChannel(
                NotificationChannel(CHANNEL_ID, "Тренировка",
    ...
    companion object {
        ...
        fun start(ctx: Context, title: String, text: String) {
            val i = Intent(ctx, HeartRateForegroundService::class.java)
                .putExtra(EXTRA_TITLE, title).putExtra(EXTRA_TEXT, text)
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) ctx.startForegroundService(i)
            else ctx.startService(i)
        }
        fun stop(ctx: Context) {
            ctx.stopService(Intent(ctx, HeartRateForegroundService::class.java))
        }
    }
}
